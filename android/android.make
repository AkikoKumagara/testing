...
$(JAR): $(JAVA_SRC) $(R_JAVA)
	$(call PRINTS,JAVAC $(subst $(ROOTDIR)/,,$?))javac -d $(CLASSPATH) \
		$(JAVAC_OPTS) -sourcepath $(ANDROID_DIR)/src:$(ANDROID_DIR)/gen $(JAVA_SRC) $(R_JAVA)
...
$(BINLIB_DIR)/libmisc.so: $(BUILDDIR)/rockbox.zip
	$(call PRINTS,CP rockbox resources)if [ -f "$(BUILDDIR)/rockbox-full.zip" ]; then cp "$(BUILDDIR)/rockbox-full.zip" $@; else cp $^ $@; fi
